...
for i = 1:length(ProjectList)
    ...
    for ii = 1:length(ExpList)
        ...
        thumbpath = fullfile(ExpList(ii).folder,ExpList(ii).name,'thumbnail.png');
        if ~exist(thumbpath,'file'); continue; end
...
bEmptyexp = false(size(List));
for i = 1:length(List)
    if isempty(List(i).Experiments)
        bEmptyexp(i) = true;
    end
end
