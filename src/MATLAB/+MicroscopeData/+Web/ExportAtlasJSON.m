...
%fullPath = fullfile(outputPath,sprintf('%s_images.json',imData.DatasetName));

% if (exist(fullfile(inputPath,'Processed',sprintf('%s_Segmenation.mat',imData.DatasetName)),'file'))
%     imData.BooleanHulls = true;
% else
%     imData.BooleanHulls = false;
% end

imData.NumberOfPartitions = 1;

if(imData.isEmpty ~= 1)
    imData.NumberOfImagesWide = imData.numImInX;
    imData.NumberOfImagesHigh = imData.numImInY;
end
